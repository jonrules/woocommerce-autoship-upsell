...
define( 'WC_Autoship_Upsell_Version', '2.0.5' );
...
//	       $item_name = $addon_license_keys[ 'wc_autoship_upsell_license_key' ]['item_name'];
	...
	if ( function_exists( 'wc_autoship_get_licensing_url' ) ) {
		...
		new WC_AS_UPSELL_Plugin_Updater( wc_autoship_get_licensing_url(), __FILE__, array(
			'version'   => WC_Autoship_Upsell_Version,
			'license'   => $license_key,
			'item_name' => $item_name,
			'author'    => 'Patterns In the Cloud'
...
add_action( 'admin_init', 'wc_as_upsell_updater', 0 );